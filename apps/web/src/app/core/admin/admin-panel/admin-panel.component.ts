...
export class AdminPanelComponent implements OnInit {
  ...
  public async rasterearEmpresas2() {
    ...
    await this.scraperRepository.createJob(url);
  }

  public async rasterearEmpresas() {
    let query = `${this.busqueda.negocio},${this.busqueda.lugar}`;
    await this.scraperRepository.createJob(query);
  }
  ...
  resizeImage(file: File): void {
    ...
    this.ng2ImgMax
      .compressImage(file, MAX_FILE_SIZE)
      .pipe(
        switchMap((result) => {
          // Obtener la imagen comprimida
          console.log(
            "Compressed image size is now " + result.size / 1024 + " kB"
          );
              ...
              return canvas.toBlob(
                (blob) => {
                  const title = this.post.title
                    ...
                    .replace(/ó/gi, "o")
                    .replace(/ú/gi, "u");
                  const newFile = new File([blob], `${title}.webp`, {
                    type: "image/webp",
                  });
      ...
      .subscribe({
        ...
        complete: () =>
          console.log(
            "Proceso de compresión y conversión de la imagen completo"
          ),
      });
  }
}
