...
async function requestIndexing(url) {
  try {
    ...
    throw new functions.https.HttpsError("internal", "Error requesting indexing");
  }
}
...
export const companyScraper = functions.runWith({
  memory: "1GB",
  timeoutSeconds: 540,
})
  .region("europe-west1")
  ...
  .onRequest(async (req, res) => {
    ...
    const worker = new Worker(__dirname + "worker_scraper.js", {workerData: query});
      ...
      if (code !== 0) {
        ...
        try {
          await Promise.all([
            ...
            writeExcel(nombreArchivo),
          ]);
          console.log(`Files ${nombreArchivo}.json and ${nombreArchivo}.xlsx written successfully`);
